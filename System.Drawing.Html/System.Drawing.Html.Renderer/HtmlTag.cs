...
namespace System.Drawing.Html.Renderer
{
  ...
  public class HtmlTag
  {
      ...
      foreach ( Match att in atts )
      {
        ...
        else if ( chunks.Length == 2 )
        {
          ...
          if ( ( attvalue.StartsWith( "\"" ) && attvalue.EndsWith( "\"" ) && attvalue.Length > 2 ) ||
              ( attvalue.StartsWith( "'" ) && attvalue.EndsWith( "'" ) && attvalue.Length > 2 ) ) //handle single quotes
          {
      ...
      foreach ( string att in Attributes.Keys )
      {
        ...
        switch ( att )
        {
          ...
          case HtmlConstants.border:
            ...
            {
              box.BorderStyle = Constants.Solid;
            }
            break;
          ...
          case HtmlConstants.nowrap:
            box.WhiteSpace = Constants.Nowrap;
            break;
    ...
    {
      ElementLength len = new ElementLength( htmlLength );
    ...
    {
      foreach ( Box box in table.Boxes )
      {
        foreach ( Box cell in box.Boxes )
        {
