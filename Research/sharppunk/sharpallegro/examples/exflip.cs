    ...
    static int Main()
    {
      ...
      while (retrace_count - c <= SCREEN_W + 32)
      {
        ...
        textprintf_ex(buffer, font, 0, 0, makecol(0, 0, 0), -1,
          "Double buffered (" + gfx_driver.name + ")");
      ...
      // TODO: replace once implemented !operator on BITMAP
      if ((page1 == IntPtr.Zero) || (page2 == IntPtr.Zero))
      {
        set_gfx_mode(GFX_TEXT, 0, 0, 0, 0);
