...
namespace System.Drawing.Html.Renderer.Css
{
    ...
    internal class LineBox
    {
        ...
        private Box _ownerBox;
        private Dictionary<Box, RectangleF> _rects;
        private List<Box> _relatedBoxes;
        ...
        {
            _rects = new Dictionary<Box, RectangleF>();
            _relatedBoxes = new List<Box>();
        ...
        /// </summary>
        public Dictionary<Box, RectangleF> Rectangles
        {
            get { return _rects; }
        }
        ...
        /// <returns></returns>
        internal List<BoxWord> WordsOf(Box box)
        {
            List<BoxWord> r = new List<BoxWord>();
        ...
        /// <param name="r"></param>
        /// <param name="b"></param>
        internal void UpdateRectangle(Box box, float x, float y, float r, float b)
        {
        ...
        {
            foreach (Box b in Rectangles.Keys)
            {
        ...
        /// </summary>
        /// <param name="g"></param>
        internal void DrawRectangles(IGraphics g)
        {
            foreach (Box b in Rectangles.Keys)
            {
                ...
                g.FillRectangle(new SolidBrush(Color.FromArgb(50, Color.Black)),
                    SWF.Rectangle.Round(Rectangles[b]));
                g.DrawRectangle(Pens.Red, SWF.Rectangle.Round(Rectangles[b]));
            }
        }
        ...
        /// <param name="g"></param>
        /// <returns></returns>
        public float GetBaseLineHeight(Box b, IGraphics g)
        {
            Font f = b.ActualFont;
            FontFamily ff = f.FontFamily;
            FontStyle s = f.Style;
            return g.GetFontHeight(f) * ff.GetCellAscent(s) / ff.GetLineSpacing(s);
        }
