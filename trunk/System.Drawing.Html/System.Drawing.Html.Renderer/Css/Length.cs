...
namespace System.Drawing.Html.Renderer.Css
{
    ...
    public class ElementLength
    {
        ...
        public ElementLength(string length)
        {
            _length = length;
            _number = 0f;
            _unit = CssUnit.None;
            ...
            if (length.EndsWith("%"))
            {
                _number = Value.ParseNumber(length, 1);
                _isPercentage = true;
                return;
            }
        ...
        public ElementLength ConvertEmToPoints(float emSize)
        {
            ...
            return new ElementLength(string.Format("{0}pt", Convert.ToSingle(Number * emSize).ToString("0.0", NumberFormatInfo.InvariantInfo)));
        }
        ...
        /// <exception cref="InvalidOperationException">If length has an error or isn't in ems</exception>
        public ElementLength ConvertEmToPixels(float pixelFactor)
        {
            if (HasError) throw new InvalidOperationException("Invalid length");
            if (Unit != CssUnit.Ems) throw new InvalidOperationException("Length is not in ems");

            return new ElementLength(string.Format("{0}px", Convert.ToSingle(Number * pixelFactor).ToString("0.0", NumberFormatInfo.InvariantInfo)));
        }
